...
# see https://twitter.com/uuunuuun1/status/803649925560971264 ..
cd Gikou-1.0.1 && \
  sed -i -e "s#\(\(params\|book\|probability\|progress\)\.bin\)#/opt/usi_engine/share/eval_dir/\1#g" src/*.cc && \
  sed -i -e 's#map_.emplace(\"BookMaxPly\", UsiOption(50, 0, 50));#map_.emplace(\"BookMaxPly\", UsiOption(70, 0, 120));#g' src/usi.cc && \
  patch -p1 < ${SCRIPT_ROOT}/engine/gikou-engine-quit_command.patch && \
  make -j ${CORE_CNT} ${MAKE_OPT} && \
  mv bin/release bin/gikou-BookMaxPly_70_0_120-v1.0.1 && \
  install -s bin/gikou-BookMaxPly_70_0_120-v1.0.1 ${DIST_ROOT}/opt/usi_engine/bin
if [ $? != 0 ]; then
  echo "build error."
  exit 1
fi

AFTER_INSTALL=${DIST_ROOT}/opt/usi_engine/share/script/after_install_gikou-BookMaxPly_70_0_120-1.0.1.sh
echo "#!/bin/sh" > ${AFTER_INSTALL}
echo "unlink /opt/usi_engine/bin/engine" >> ${AFTER_INSTALL}
echo "ln -s /opt/usi_engine/bin/gikou-BookMaxPly_70_0_120-v1.0.1 /opt/usi_engine/bin/engine" >>${AFTER_INSTALL}
...
fpm --url "https://github.com/gikou-official/Gikou"  \
	...
	-s dir -t deb -C ${DIST_ROOT} -v 1.0.1 -n gikou-engine-BookMaxPly_70_0_120-${MAKE_OPT} -p gikou-engine-BookMaxPly_70_0_120-1.0.1-${MAKE_OPT}.deb .

mv gikou-engine-BookMaxPly_70_0_120-1.0.1-${MAKE_OPT}.deb ../
